...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["nombre"]) && isset($_POST["apellido"]) && isset($_POST["email"]) && isset($_POST["telefono"]) && isset($_POST["id_usuario"])) {
    ...
    // CORREGIDO: Verificar si el nuevo email o teléfono ya están en uso por OTRO usuario
    if (empty($errores)) {
        $sql_check = $conexion->prepare("SELECT id FROM usuarios WHERE (email = :email OR telefono = :telefono) AND id != :id_usuario LIMIT 1");
        $sql_check->bindParam(':email', $email, PDO::PARAM_STR);
        $sql_check->bindParam(':telefono', $telefono, PDO::PARAM_STR);
        $sql_check->bindParam(':id_usuario', $id_usuario, PDO::PARAM_INT);
        $sql_check->execute();
        if ($sql_check->fetch()) {
            $errores[] = "El email o el teléfono ya están registrados por otro usuario.";
        }
    }
